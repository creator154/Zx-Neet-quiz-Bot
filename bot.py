...
from telegram import Update, Poll, InlineKeyboardButton, InlineKeyboardMarkup, KeyboardButton, KeyboardButtonPollType, ReplyKeyboardMarkup, ReplyKeyboardRemove
from telegram.ext import Application, CommandHandler, MessageHandler, PollAnswerHandler, ConversationHandler, CallbackQueryHandler, filters, ContextTypes
...
async def save_desc_or_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Direct poll button for first question
...
# Step 4: After /done → Timer selection
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("Select time per question:", reply_markup=reply_markup)
    return TIMER

async def set_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    timer_map = {"10 sec":10, "15 sec":15, "20 sec":20, "30 sec":30, "45 sec":45, "60 sec":60}
    context.user_data['timer'] = timer_map.get(text, DEFAULT_TIMER)
    ...
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("Shuffle questions and options?", reply_markup=reply_markup)
    return SHUFFLE
...
async def set_shuffle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [KeyboardButton("Negative 0"), KeyboardButton("Negative 0.5"), KeyboardButton("Negative 1")]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("Select negative marking per wrong answer:", reply_markup=reply_markup)
    return NEGATIVE
...
async def set_negative(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inline button to directly start quiz in group (callback)
...
# Callback for inline "Start Quiz in Group"
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("start_"):
        ...
        # Must be in group, start first question
...
async def send_next(context: ContextTypes.DEFAULT_TYPE, chat_id:int):
    ...
    await context.bot.send_poll(chat_id, question=q['question'], options=q['options'], type=Poll.QUIZ,
                                correct_option_id=q['correct'], explanation=q['explanation'],
                                is_anonymous=False, open_period=quiz.get('timer', DEFAULT_TIMER))
    active['index'] += 1
